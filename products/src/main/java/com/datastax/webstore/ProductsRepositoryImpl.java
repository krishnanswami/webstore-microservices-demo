...
public class ProductsRepositoryImpl implements ProductsRepository {
    ...
    public Optional<Products> findByItemId(String id) {
        return Optional.ofNullable(
                cqlSession.execute(SimpleStatement.newInstance("SELECT * from catalog.products where inventory_itemid = ?", id)).one()
        ).map(this::mapToProducts);
    }

    @Override
    public Products insert(Products Products) {
        cqlSession.execute(SimpleStatement.newInstance("INSERT INTO catalog.products ( inventory_itemid, catalog_name, catalog_description, catalog_price, inventory_quantity) VALUES (?,?,?,?,?)",
    ...
    public void deleteByItemId(String id) {
        cqlSession.execute(SimpleStatement.newInstance("DELETE from catalog.products where inventory_itemid = ?", id));
    }

    public void deleteAll() {
        cqlSession.execute(SimpleStatement.newInstance("TRUNCATE catalog.products"));
    }

    @Override
    public List<Products> findAll() {
        ResultSet result = cqlSession.execute(SimpleStatement.newInstance("SELECT * FROM catalog.products"));
        return result.all().stream().map(this::mapToProducts).collect(Collectors.toList());
    }

    @Override
    public Products update(Products Products) {
        cqlSession.execute(SimpleStatement.newInstance("UPDATE catalog.products SET catalog_name = ?, catalog_description = ?, catalog_price = ?, inventory_quantity = ? WHERE inventory_itemid = ?",
